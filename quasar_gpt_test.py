...
def ChatGPT(user_query):
    ...
    completion = client.chat.completions.create(model=model_to_use,
		messages=[
    ...
    if user_query != "":
        ...
        with st.spinner(text="Processing..."):
            ...
            if try_to_plot:
                query_to_send = (
                    query_to_send
                    + " - the resulting table should have two columns, 'x' and 'y', so that I can plot this as a line chart"
                )
            response = ChatGPT(query_to_send).content
            sql_query = response
            if "```sql" in sql_query:
                sql_query = sql_query[
                    sql_query.find("```sql") + len("```sql") : sql_query.rfind("```")
                ]
            st.code(sql_query, language="sql")
            df_to_display = get_data(sql_query)
            st.download_button(
                label="Download data as CSV",
                data=df_to_display.to_csv(index=False),
                file_name="quasar_gpt_output.csv",
                mime="text/csv",
            )
        st.success("Done!")
...
user_query = st.text_input(
    "Enter query here", placeholder="for example: # of signups last week"
)
